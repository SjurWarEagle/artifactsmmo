...
public abstract class CommonBrain implements Brain {
    ...
    protected CommonBrain(Caches caches, WishList wishList, ApiHolder apiHolder, CharHelper charHelper,
                          BankFetchItemsAndCraftTask bankFetchItemsAndCraftTask, MapHelper mapHelper) {
        this.caches = caches;
    ...
    public void craftGearIfNotAtCharacter(String characterName, String gear, String craftingStation, ItemSlot slot) {
        CharacterSchema character = apiHolder.charactersApi.getCharacterCharactersNameGet(characterName)
                                                           .getData();
        boolean enoughResourcesToCraft = character.getInventory()
                                                  .stream()
        ...
        CraftingSchema craftingSchema = new CraftingSchema().code(gear)
                                                            .quantity(1);
        apiHolder.myCharactersApi.actionCraftingMyNameActionCraftingPost(character.getName(), craftingSchema
        );
