...
public class MinerT2Brain extends CommonBrain {
    ...
    private boolean hasItemsToCraftInInventory(ItemSchema item, List<InventorySlot> inventory) {
        boolean hasItemsToCraftInInventory = true;
        for (SimpleItemSchema simpleItemSchema : item.getCraft()
                                                     .getItems()) {
    ...
    public void runBaseLoop(String characterName) throws BrainCompletedException {
        while (true) {
            ...
            boolean allCopperEquipped = hasMaxCraftableGearEquipped(character.getData()
                                                                             .getName());
