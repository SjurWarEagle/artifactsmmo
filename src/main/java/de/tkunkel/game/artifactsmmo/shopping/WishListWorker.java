...
public class WishListWorker {
    ...
    public WishListWorker(WishList wishList, CharHelper charHelper, ItemHelper itemHelper, HuntForItemTask huntForItemTask, CombatSimulator combatSimulator, BankDepositAllTask bankDepositAllTask, BankFetchItemsAndCraftTask bankFetchItemsAndCraftTask, FarmResourceTask farmResourceTask, MyAccountApiWrapper myAccountApi, BuyItemForTokenTask buyItemForTokenTask, MonsterHelper monsterHelper, ApiHolder apiHolder, NpcHelper npcHelper, MapHelper mapHelper) {
    ...
    public synchronized Optional<Wish> reserveWishThatCanBeHuntedByMe(CharacterSchema character) {
        ...
        var newWish = wishList.allWishes.stream()
    ...
    public synchronized Optional<Wish> reserveWishThatCanBoughtByMe(CharacterSchema character) {
        ...
        for (Wish wish : wishList.allWishes) {
            if (!wish.fulfilled
                    && wish.reservedBy == null) {
                Optional<ItemSchema> itemDefinition = itemHelper.findItemDefinition(wish.itemCode);
    ...
    private @NonNull Optional<Wish> getAlreadyReservedWish(CharacterSchema character, String wishType) {
        Optional<Wish> existingReservedWish = wishList.allWishes.stream()
                                                                .filter(wish -> !wish.fulfilled)
